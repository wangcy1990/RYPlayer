...
typedef void (^RYDownLoadDataCallBack)(NSData *data, NSError *error);
typedef void (^RYDownloadProgressBlock)(unsigned long long total, unsigned long long current);

@interface RYImageDownloader : NSObject<NSURLSessionDownloadDelegate>
...
@property (nonatomic, copy) RYDownloadProgressBlock progressBlock;
@property (nonatomic, copy) RYDownLoadDataCallBack callbackOnFinished;

- (void)startDownloadImageWithUrl:(NSString *)url
                         progress:(RYDownloadProgressBlock)progress
                         finished:(RYDownLoadDataCallBack)finished;

@end

typedef void (^RYImageBlock)(UIImage *image);

@interface UIImageView (RYCache)
 ...
 */
@property (nonatomic, copy) RYImageBlock completion;

/**
 *  Image downloader
 */
@property (nonatomic, strong) RYImageDownloader *imageDownloader;
